...
class L0Module(nn.Module):
    ...
    def __init__(self, config, 
        ...
        self.params_per_mlp_layer = self.hidden_size * self.intermediate_size * 2 + self.hidden_size + self.intermediate_size
        ...
        self.loga_mean = math.log(1.0 - self.epsilon - init_loga) - math.log(init_loga + self.epsilon)
    ...
    def initialize_one_module(self, module_name):
        ...
        if module_name == "intermediate":
            self.intermediate_loga = self.initialize_parameters(self.intermediate_size, self.num_hidden_layers, mean=default_mean)
            self.add_one_module(
            ...
            self.hidden_loga = self.initialize_parameters(self.hidden_size, mean=default_mean)
            self.add_one_module(
            ...
            self.ffn_loga = self.initialize_parameters(self.num_hidden_layers, mean=default_mean)
            self.add_one_module(
                self.ffn_loga, type_name="ffn", 
                parameter_per_dim=self.params_per_mlp_layer, size=1,
                shape=[self.num_hidden_layers]
            )
            self.mha_loga = self.initialize_parameters(self.num_hidden_layers, mean=default_mean)
            self.add_one_module(
    ...
    def add_one_module(self, z_loga, type_name, parameter_per_dim, size, shape): #! init the z_logas
    ...
    def constrain_parameters(self):
        for key in self.z_logas:
            self.z_logas[key].data.clamp_(min=math.log(1e-2), max=math.log(1e2))
    ...
    def get_num_parameters_and_constraint(self, hidden=False):
        ...
        # 12 * 1 * 1
        mha_score = self.score_loga(self.mha_loga).view(-1, 1, 1) if "mha" in self.types else torch.ones([layers, 1, 1]).to(device)
        # 12 * 12 * 1
        heads_score = self.score_loga(self.heads_loga).unsqueeze(dim=-1) if "heads" in self.types else torch.ones([layers, heads, 1]).to(device)
        ...
        if hidden:
            hidden_score = self.score_loga(self.hidden_loga) if "hidden" in self.types else torch.ones([hidden_size]).to(device)
            heads_score = (heads_score * mha_score) if mha_score is not None else heads_score #38+106
            heads_score = heads_score.reshape(-1)
            num_parameters += torch.outer(hidden_score, heads_score).sum() * self.parameters_per_dim["heads"] / self.hidden_size
        else:
            heads_score = heads_score * mha_score
            num_parameters += heads_score.sum() * self.parameters_per_dim["heads"]

        # 12 * 1
        if 'ffn' in  self.types:
            ffn_score = self.score_loga(self.ffn_loga).unsqueeze(dim=-1) if "ffn" in self.types else torch.ones([layers, 1]).to(device)
        else:
            ffn_score = 1
        # 12 * 3072
        intermediate_score = self.score_loga(self.intermediate_loga) if "intermediate" in self.types else torch.ones([layers, hidden_size*4]).to(device)

        intermediate_score = intermediate_score * ffn_score

        if hidden:
            intermediate_score = intermediate_score.reshape(-1)#13893+22971
            num_parameters += torch.sum(torch.outer(hidden_score, intermediate_score)) * 2
        else:
            num_parameters += intermediate_score.sum() * self.parameters_per_dim["intermediate"]
    ...
    def _sample_z(self, loga):
        ...
        # quantile concrete
        z = torch.sigmoid((torch.log(u) - torch.log(1 - u) + loga) / self.temperature)
    ...
    def get_z_from_zs(self, zs):
        ...
        for t in self.types:
            name = t
            numpified_zs[name] = (zs[t].squeeze().detach().cpu().numpy() > 0) if t in zs else np.ones(self.shapes[name])
        return numpified_zs
    ...
    def calculate_model_size(self, zs):
        ...
        hidden_z = numpified_zs["hidden"] if "hidden" in numpified_zs.keys() else np.ones([hidden_size])
        heads_z = numpified_zs["heads"] if "heads" in numpified_zs.keys() else np.ones([layers,1,heads,1,1])
        mha_z = numpified_zs["mha"].reshape(-1, 1, 1, 1, 1) if "mha" in numpified_zs.keys() else np.ones([heads_z.shape[0], 1, 1, 1, 1])
        intermediate_z = numpified_zs["intermediate"]  if "intermediate" in numpified_zs.keys() else np.ones([layers,1,1,hidden_size*4])
        ffn_z = numpified_zs["ffn"].reshape(-1, 1, 1, 1) if "ffn" in numpified_zs.keys() else np.ones([heads_z.shape[0], 1, 1, 1])

        remain_hidden = hidden_z.sum().item()
        remain_intermediate = intermediate_z.reshape(self.num_hidden_layers, self.intermediate_size).sum(-1).tolist()
        remain_heads = heads_z.reshape(self.num_hidden_layers, self.num_attention_heads).sum(-1).tolist()

        heads = np.outer((heads_z * mha_z).reshape(-1), hidden_z).sum().item()
        intermediate = np.outer((intermediate_z * ffn_z).reshape(-1), hidden_z).sum().item()
...
if __name__ == '__main__':
    ...
    Config = namedtuple('Config', ['hidden_size', 'intermediate_size', 'num_attention_heads', 'num_hidden_layers'])
    config = Config(hidden_size=768, intermediate_size=4*768, num_attention_heads=12, num_hidden_layers=12)
